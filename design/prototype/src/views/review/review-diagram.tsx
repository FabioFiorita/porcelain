...
import type { Diagram, DiagramBox } from '../../contracts/review';
...
function Box({ data }: NodeProps<Node<BoxData>>) {
  ...
  return (
    <div
      ...
      className={cn(
        'rounded-xl border bg-card px-3 py-2 text-card-foreground shadow-xs transition-[box-shadow,border-color]',
  ...
  const edges: Edge[] = graph.arrows
    ...
    .map((arrow, index) => {
      ...
      return {
        ...
        style: arrow.dashed ? { strokeDasharray: '4 3' } : undefined,
        focusable: false,
      };
    });
  return { nodes, edges };
