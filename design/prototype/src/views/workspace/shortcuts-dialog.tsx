...
/** `Alt+Shift+R` → one key cap per key, each in the platform's own notation. */
...
export function ShortcutsDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85svh] grid-rows-[auto_minmax(0,1fr)] gap-5 overflow-hidden sm:max-w-xl">
        <DialogHeader className="flex-row items-center gap-3 text-left">
          <DialogIcon icon={Keyboard} />
          <div className="flex flex-col gap-0.5">
            <DialogTitle>Keyboard shortcuts</DialogTitle>
            <DialogDescription>
              Plain keys like J or C don’t fire while you are typing.
            </DialogDescription>
          </div>
        </DialogHeader>
