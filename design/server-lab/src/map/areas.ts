...
const connection: Area = {
    ...
    },
    {
      ...
      steps: [
        ...
        s(
          'route',
          'browserSessionValid',
          `Without an Authorization header, accepts one porcelain_session cookie whose expiry and HMAC (keyed by the server token) are valid, but only with the browser header.`,
          'apps/server/src/http/middlewares/browser-session.ts',
          13,
        ),
        s(
          ...
          `Otherwise compares the Authorization header with the token in constant time.`,
          'apps/server/src/http/middlewares/authenticate.ts',
          14,
        ),
        s(
      ...
      gitCommands: [],
      tables: [],
      cost: 'Constant: one HMAC or one constant-time compare per request.',
      notes: `The MCP group adds a hook that rejects requests without an Authorization header, so cookies never authenticate agents.`,
    },
    {
    ...
    },
    {
      ...
      webTriggers: [
        wt(
          ...
          web('query/workspace-provider.tsx'),
          112,
          `On Disconnect, after saving open file drafts; then aborts the connection and clears the query cache.`,
        ),
      ],
      steps: [
        s(
        ...
        s(
          'route',
          'clearBrowserSession',
          `Sets porcelain_session with Max-Age=0.`,
          'apps/server/src/http/middlewares/browser-session.ts',
          41,
        ),
      ],
      ...
      tables: [],
      cost: 'Constant.',
      notes: `Not authenticated. It cannot revoke a copied cookie: sessions end only by expiry or token rotation.`,
    },
    {
  ...
  observations: [
    ...
    {
      kind: 'question',
      title: 'Logout does not revoke',
      detail: `The cookie carries its own expiry (30 days) and a token-keyed HMAC; there is no session table. A copied cookie stays valid until expiry unless the token is rotated. Documented, but worth deciding before remote access.`,
      sources: [
        at('apps/server/src/http/middlewares/browser-session.ts', 4),
        at('apps/server/src/http/middlewares/browser-session.ts', 22),
      ],
      confidence: 'verified',
    },
    {
      ...
      detail: `HttpOnly, SameSite=Strict, scoped to /api, and only honoured with the x-porcelain-browser header, which a cross-origin page cannot send without CORS.`,
      sources: [
        at('apps/server/src/http/middlewares/browser-session.ts', 14),
        at('apps/server/src/http/middlewares/authenticate.ts', 9),
      ],
      confidence: 'verified',
    },
  ],
  ...
  flows: [
    ...
    },
    {
      ...
      webTriggers: [
        wt(
          'useReviewSummaries',
          web('query/review-summaries.ts'),
          32,
          `ConnectedWorkspace (connected-workspace.tsx:65) mounts one query per available worktree of every project. readInBackground (review-summaries.ts:10) chains them per connection, so only one is in flight. staleTime 30 s, refetch on focus when stale. Invalidated by comment writes (comments.ts:52), the end of a reviewed-mark queue (reviewed-queue.ts:113), file edits (files.ts:46) and Git receipts needing refresh (git-actions.ts:53). The first results also pick the fallback worktree (connected-workspace.tsx:66).`,
        ),
      ],
      steps: [
        s(
          'route',
          'listReviewedFiles GET review-summary',
          `Returns { worktreeId, pendingFiles, openThreads }.`,
        ...
        s(
          ...
          `Runs on the summaries queue, beside the main queue.`,
          APP,
          234,
        ),
        s(
        ...
        s(
          'use-case',
          'ReadReviewSummary.execute',
          `Loads reviewed marks, then chooses status or evidence.`,
          caseFile('read-review-summary.ts'),
          22,
        ),
        s(
          'use-case',
          'ReadWorktreeStatus.execute',
          `No stored marks: counts changed paths from a status read.`,
          caseFile('read-review-summary.ts'),
          30,
        ),
        s(
          ...
          `Any stored mark: reads full evidence (shared cache) and counts paths whose fingerprint differs from the mark.`,
          caseFile('read-review-summary.ts'),
          37,
        ),
        s(
      ...
      runner: 'summaries',
      gitCommands: [
        `Without marks: ${STATUS_LINE}`,
        'With any stored mark: full evidence read (see changes.evidence): 13 on a cache hit, about 34 + N on a miss',
      ],
      tables: [
        { name: 'project_worktrees', access: 'read' },
      ...
      cost: `Per available worktree, strictly serial: 13 Git processes without marks, 13 to about 34 + N with marks. Connect and every stale focus cost K times that.`,
    },
  ],
  decisions: [
    {
    ...
    {
      title: 'Refresh on every query run',
      summary: `Focus and reconnect are treated as the natural moment to discover new worktrees, so the inventory query always posts refresh. Every focus therefore costs a Git rescan of all projects on the main queue.`,
      source: at(web('query/inventory.ts'), 62),
    },
    {
      title: 'Summaries on their own queue, serialized twice',
      summary: `Badges run on a dedicated summaries runner and the client chains them, so a burst of worktrees cannot flood Git or block the main queue. Badges fill in one worktree at a time, and they still run Git concurrently with main-queue work.`,
      source: at(web('query/review-summaries.ts'), 10),
    },
  ],
  observations: [
    {
    ...
    },
    {
      kind: 'performance',
      title:
        'One stored reviewed mark forces a full evidence read for the badge',
      detail: `The summary reads status only when the worktree has no marks; otherwise it runs full evidence. Marks are removed only by unmarking, by the 2000-mark eviction or by project removal, never on commit. After the first review, every badge of that worktree takes the evidence path, even when its marked files are long committed.`,
      sources: [
        at(caseFile('read-review-summary.ts'), 29),
        at(caseFile('read-review-summary.ts'), 37),
        at(repo('reviewed-file-repository.ts'), 72),
        at(repo('reviewed-file-repository.ts'), 98),
      ],
      confidence: 'verified',
    },
    {
      kind: 'performance',
      title: 'Sidebar badges are K sequential Git reads',
      detail: `One summary per available worktree, chained on the client and single-lane on the server. Ten worktrees without marks is about 130 Git processes before all badges show; with marks and a busy agent (cache misses) much more. The fallback worktree selection on connect depends on these results.`,
      sources: [
        at(web('query/review-summaries.ts'), 45),
        at(APP, 234),
        at(web('views/workspace/connected-workspace.tsx'), 66),
      ],
      confidence: 'verified',
    },
    {
...
const commentWriteWeb = (hook: string, line: number, when: string) =>
  wt(
    ...
    line,
    `${when} Commands are serialized per worktree (enqueueComment, comments.ts:72); the result is merged into the cache and the worktree's summary is invalidated (comments.ts:52), which re-runs Git on the summaries queue.`,
  );

const comments: Area = {
  id: 'comments',
  title: 'Comment threads',
  webSurface: `File discussion under the file header in Files and Changes, and open-thread counts in the sidebar badges.`,
  summary: `Threads are JSON rows keyed by worktree ID, limited to 100 threads, 100 messages each and 1 MiB per worktree. Listing is a synchronous SQLite read outside the queue; writes go through the operations queue. HTTP writes are attributed to the reviewer and MCP writes to the agent. Every web comment write also invalidates the worktree's summary, which re-runs Git just to refresh an open-thread count.`,
  flows: [
    {
      id: 'comments.list',
      title: 'List threads',
  ...
  observations: [
    {
      kind: 'performance',
      title: 'A comment write re-runs Git for a badge',
      detail: `Each create, reply or resolve invalidates the worktree summary, and the summary recomputes status or full evidence just to update openThreads, which is a SQLite count. The count could be patched locally or served by a SQLite-only endpoint.`,
      sources: [
        at(web('query/comments.ts'), 52),
        at(caseFile('read-review-summary.ts'), 29),
      ],
      confidence: 'verified',
    },
...
const mcp: Area = {
  ...
  flows: [
    {
      ...
      steps: [
        s(
          'route',
          'mcpRoutes bearer hook',
          `Rejects requests without an Authorization header before authenticate runs.`,
          route('mcp.ts'),
          12,
        ),
        s(
          'route',
          'origin check',
          `Any Origin header gets 403; non-POST methods get 405.`,
          route('mcp.ts'),
          21,
        ),
        s(
  ...
  decisions: [
    {
      title: 'Agents connect over MCP with the bearer; nothing is pushed',
    ...
    },
    {
      ...
      source: at(route('mcp.ts'), 27),
    },
    {
      title: 'Browser requests are refused at the agent endpoint',
      summary: `An Origin header means a browser, which gets 403, and a bearer header is mandatory, so cookies can never drive agent tools.`,
      source: at(route('mcp.ts'), 21),
    },
  ],
  observations: [
    {
    ...
    },
    {
      ...
      detail: `Bearer-only, Origin rejected, per-request server, errors mapped to the same safe API errors as HTTP.`,
      sources: [at(route('mcp.ts'), 12), at(MCP_SERVER, 163)],
      confidence: 'verified',
    },
  ],
};
  ...
  flows: [
    {
      ...
      steps: [
        ...
        s(
          'application',
          'runLocalServer',
          `Ensures the token, starts the server and waits for shutdown.`,
        ...
        s(
          'filesystem',
          'ensureAccessToken',
          `Creates a 0600 token file once, or reads and validates the existing one.`,
          'apps/server/src/cli/token.ts',
          57,
        ),
        s(
          'application',
