...
description: Reviews the current working-tree diff against Porcelain's earned invariants (the audit skill), the one client architecture, and the AGENTS.md hard rules. Use before committing a non-trivial change, or whenever asked to check a diff for regressions. Read-only — reports findings, never edits.
tools: Read, Grep, Glob, Bash
model: inherit
---

<!-- Canonical vendor-neutral source. pnpm agents:sync writes the Codex adapter; Claude/Grok use a symlink. -->
