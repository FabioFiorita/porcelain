...
- **A terminal is a `TabKind`, so split view + tabs come for free** — no bespoke panel. Terminal tabs open **pinned** (a click must not replace a running shell like a preview tab). The `path` field holds the session id.
