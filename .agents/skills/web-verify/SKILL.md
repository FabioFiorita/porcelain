...
3. Write the spec `apps/web/spec/browser/<domain>-<capability>.browser.ts`. Import `test` from `../kit/journey`, `expect` from `vitest`, and take the fixtures the journey needs:
   ...
   - `agent`: the coding agent's side: `publishReview(title)` publishes a one-layer review over the sample change and `comment(path, body)` comments on a file as the agent, both through the review tools on the owner socket, and `editFile(edit)` changes the worktree through the files API as another client;
