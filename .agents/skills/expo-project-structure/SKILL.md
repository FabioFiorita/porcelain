...
Agent instructions live at the repo root — `AGENTS.md` / `CLAUDE.md`, with canonical project skills under `.agents/skills/`. Porcelain exposes Claude-compatible symlink adapters under `.claude/skills/`; never make those a second source of truth. Other config and assets stay outside `src/`: `app.json` / `app.config.ts`, `eas.json`, `package.json`, `assets/`, and `scripts/`.
