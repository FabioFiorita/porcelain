...
if (feature !== 'projects.rename' || process.argv.length !== 3) {
  process.stderr.write('Usage: node .agents/skills/server-verify/scripts/verify.ts projects.rename\n');
  process.exit(2);
}

const evidenceDirectory = await mkdtemp(join(tmpdir(), 'porcelain-server-verify-'));
...
function check(testCase: Case, name: string, expected: unknown, actual: unknown): void {
...
async function exchange(
  ...
  return {
    request: { method, path, ...(body === undefined ? {} : { body }), authorization: 'Bearer [redacted]' },
...
function inventoryOf(entry: Exchange): RecordValue {
  ...
  if (entry.response.status !== 200 || !object(body) || !Array.isArray(body.projects))
...
async function verifyInvalidInput(
  ...
  const rejected = await exchange(address, 'PATCH', `/api/projects/${projectId}`, {
    name: submittedName,
  });
  ...
  check(testCase, 'invalid input error code', 'INVALID_REQUEST',
    object(rejected.response.body) ? rejected.response.body.code : undefined);
...
async function verifyRename(address: string): Promise<void> {
  const success: Case = { name: 'existing project', exchanges: [], assertions: [] };
  cases.push(success);
  const beforeRead = await exchange(address, 'GET', '/api/inventory');
  ...
  const renamed = await exchange(address, 'PATCH', `/api/projects/${project.id}`, {
    name: '  New name  ',
  });
  ...
  const after = inventoryOf(afterRead);
  const renamedProject = projectsOf(after).find((entry) => entry.id === project.id);
  check(success, 'rename status', 200, renamed.response.status);
  check(success, 'rename response', { id: project.id, name: 'New name' }, renamed.response.body);
  check(success, 'renamed project remains in inventory', true, renamedProject !== undefined);
  check(success, 'inventory changes only the project name',
    { ...before, projects: projects.map((entry) => entry.id === project.id ? { ...entry, name: 'New name' } : entry) },
    after);

  const missing: Case = { name: 'unknown project', exchanges: [], assertions: [] };
  cases.push(missing);
  ...
  const rejected = await exchange(address, 'PATCH', `/api/projects/${unknownId}`, {
    name: 'Ghost',
  });
  ...
  check(missing, 'unknown project error code', 'PROJECT_NOT_FOUND',
    object(rejected.response.body) ? rejected.response.body.code : undefined);
  check(missing, 'unknown project leaves inventory unchanged', missingBefore, missingAfter);

  await verifyInvalidInput(address, 'empty name', project.id, '');
  await verifyInvalidInput(address, 'whitespace-only name', project.id, '   ');
  await verifyInvalidInput(address, 'name over 100 characters', project.id, 'x'.repeat(101));
  await verifyInvalidInput(address, 'control character in name', project.id, 'Line\nbreak');
  await verifyInvalidInput(address, 'invalid project ID', 'not-a-uuid', 'Valid name');
}

async function waitForReady(): Promise<Ready> {
  return new Promise((resolveReady, rejectReady) => {
    let pending = '';
    const timeout = setTimeout(() => rejectReady(new Error('Isolated server did not become ready within 30 seconds')), 30_000);
    const onExit = (code: number | null) => {
      clearTimeout(timeout);
      child.off('error', onError);
      rejectReady(new Error(`Isolated server exited before ready: ${code ?? 'signal'}`));
    };
    ...
    child.stdout.on('data', (chunk: Buffer) => {
      ...
      for (let newline = pending.indexOf('\n'); newline !== -1; newline = pending.indexOf('\n')) {
        ...
        try {
          ...
          if (object(value) && typeof value.manifest === 'string' && typeof value.address === 'string' && typeof value.repository === 'string') {
            clearTimeout(timeout);
            child.off('close', onExit);
            child.off('error', onError);
...
try {
  ...
  const manifest = JSON.parse(await readFile(ready.manifest, 'utf8')) as Manifest;
  const secret = JSON.parse(await readFile(manifest.credentialFile, 'utf8')) as { credential?: string };
  if (!secret.credential || manifest.address !== ready.address || manifest.repository !== ready.repository)
    throw new Error('Isolated server session manifest is incomplete');
  credential = secret.credential;
  await verifyRename(manifest.address);
} catch (error) {
  ...
  const closed = await Promise.race([
    ...
    new Promise<{ closed: false }>((resolveTimeout) => {
      shutdownTimer = setTimeout(() => resolveTimeout({ closed: false }), 10_000);
    }),
  ]);
...
const passed = errorMessage === undefined && assertions.length > 0 && assertions.every((entry) => entry.passed);
const evidence = {
  feature,
  ...
  cases,
  session: ready ? { address: ready.address, repository: ready.repository } : null,
  logs: { stdout: scrub(stdout), stderr: scrub(stderr) },
};
await writeFile(evidencePath, `${JSON.stringify(evidence, null, 2)}\n`, { mode: 0o600 });
process.stdout.write(`${passed ? 'PASS' : 'FAIL'} ${feature}: ${evidence.passedAssertions}/${evidence.assertionCount} assertions; evidence: ${evidencePath}\n`);
