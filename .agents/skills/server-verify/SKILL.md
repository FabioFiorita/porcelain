---
name: server-verify
description: Run Porcelain's isolated server and collect repeatable HTTP evidence for mapped server features. Use when verifying server behavior or a server change; current coverage is project rename only.
---
...
For project rename, run from the repository root:
