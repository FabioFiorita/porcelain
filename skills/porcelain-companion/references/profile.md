...
~/.porcelain/porcelain profile set --profile -            # { pinnedPaths, hiddenPaths, layers }

~/.porcelain/porcelain worktree profile get               # baseline + override + the merge
~/.porcelain/porcelain worktree profile set --profile -   # + unhiddenPaths
...
- **Never write a profile unasked.** Porcelain ships the mechanism, not the
  policy (ADR 0003). Write one when the human asks, or when their own agent
...
- **Nothing here is shared.** Both levels are personal (ADR 0006) and neither is
