...
private final class TerminalInputField: UITextField {
  ...
  var onHardwareArrow: ((String) -> Void)?
  ...
  private func handleHardwareKeyCommand(_ command: UIKeyCommand) {
    ...
    case UIKeyCommand.inputUpArrow,
      ...
      UIKeyCommand.inputLeftArrow,
      UIKeyCommand.inputRightArrow:
      // JS owns the shared key encoder and knows DECCKM from its companion parser. A static
      // ESC[A here breaks vim and other applications that enabled application cursor keys.
      onHardwareArrow?(input)
      return
    default:
      break
    }
...
public final class PorcelainTerminalView: ExpoView, UITextFieldDelegate {
  ...
  var terminalKey: String = "" {
    didSet {
      accessibilityIdentifier = "porcelain-terminal-\(terminalKey)"
  ...
  public required init(appContext: AppContext? = nil) {
    ...
    inputField.onHardwareArrow = { [weak self] input in
      self?.emitHardwareArrow(input)
    }

    focusTapGesture.addTarget(self, action: #selector(handleViewportTap))
    // Selection intentionally stays outside Ghostty's clipboard callbacks. JS owns the
    // replay-aware OSC 52 path, so replay never re-writes the system pasteboard.
    selectionLongPressGesture.addTarget(self, action: #selector(handleSelectionLongPress(_:)))
    selectionLongPressGesture.minimumPressDuration = 0.35
    selectionLongPressGesture.cancelsTouchesInView = true
  ...
  public override func canPerformAction(_ action: Selector, withSender sender: Any?) -> Bool {
    if action == #selector(copySelection(_:)) {
  ...
  /** Long-press drag selection plus the platform Copy menu. */
  @objc
  ...
  @objc
  private func copySelection(_ sender: Any?) {
    guard let surface else { return }
    var text = ghostty_text_s()
    guard ghostty_surface_read_selection(surface, &text), let pointer = text.text else { return }
    defer { ghostty_surface_free_text(surface, &text) }
    let data = Data(bytes: UnsafeRawPointer(pointer), count: Int(text.text_len))
    guard let value = String(data: data, encoding: .utf8), !value.isEmpty else { return }
    UIPasteboard.general.string = value
  }

  private func showSelectionMenu(at location: CGPoint) {
    guard let surface, ghostty_surface_has_selection(surface), becomeFirstResponder() else { return }
    UIMenuController.shared.showMenu(
      from: terminalViewport,
      rect: CGRect(origin: location, size: .zero)
    )
  }
  ...
  private func setupWriteCallback() {
    ...
    ghostty_surface_set_write_callback(surface, { userdata, data, len in
      ...
      let view = Unmanaged<PorcelainTerminalView>.fromOpaque(userdata).takeUnretainedValue()
  ...
  private func emitHardwareArrow(_ input: String) {
    let key: String
    switch input {
    case UIKeyCommand.inputUpArrow:
      key = "up"
  ...
  private func writeThemeConfigFile() -> String? {
    ...
    let url = URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("porcelain-terminal-theme-\(appearance.rawValue).ghostty")
