...
class PorcelainTerminalView(context: Context, appContext: AppContext) : ExpoView(context, appContext) {
  ...
  var terminalKey: String = ""
    set(value) {
      ...
      field = value
      contentDescription = "porcelain-terminal-$value"
      recreateTerminal()
    }
  ...
  var scrollLines: Int = 0
    set(value) {
      field = value
      pendingScrollLines = value
    }

  var scrollRequest: Double = 0.0
    set(value) {
      val previous = field
      field = value
      if (value != previous && terminalHandle != 0L && pendingScrollLines != 0) {
        GhosttyBridge.nativeScroll(terminalHandle, pendingScrollLines)
        renderSnapshot()
      }
    }
  ...
  init {
    ...
    terminalCanvas.onScrollRows = { delta ->
      // JS owns the shared normal-vs-alternate policy. It returns normal scroll requests
      // through props; alternate pans become real wheel/Page input to the remote PTY.
      onScroll(mapOf("lines" to delta))
    }
    ...
    inputView.setOnKeyListener { _, keyCode, event ->
      ...
      when {
        ...
        keyCode == KeyEvent.KEYCODE_TAB -> {
          onInput(mapOf("data" to if (event.isShiftPressed) "\u001B[Z" else "\t"))
          true
        }
        keyCode == KeyEvent.KEYCODE_DPAD_UP -> {
          onKey(mapOf("key" to "up"))
          true
        }
        keyCode == KeyEvent.KEYCODE_DPAD_DOWN -> {
          onKey(mapOf("key" to "down"))
          true
        }
        keyCode == KeyEvent.KEYCODE_DPAD_LEFT -> {
          onKey(mapOf("key" to "left"))
          true
        }
        keyCode == KeyEvent.KEYCODE_DPAD_RIGHT -> {
          onKey(mapOf("key" to "right"))
          true
        }
