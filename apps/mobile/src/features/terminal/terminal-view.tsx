 ...
 * mounts the session on screen and component state would destroy the shell (and its
 * scrollback) on every switch. All this does is size the grid to the pane, paint the current
 * viewport, and route touches and keystrokes.
 *
...
export function TerminalView({ sessionId }: { sessionId: string }): React.JSX.Element {
  ...
  const gridHeight = Math.max(0, pane.height - keyboardInset)
