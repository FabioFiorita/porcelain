...
/**
 * Replay a re-attached session's scrollback. Seeds at most once per session, for the same
 * reason the desktop client does: the first attach after a cold start writes the history, and
 * every later re-attach is a socket detail the emulator already knows nothing about.
 */
