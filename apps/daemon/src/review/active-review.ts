...
import { type FlowGroup, groupByLayerOrdered, type Layer, resolveImport } from './flow'
import type { FileSource, ReviewSection, ReviewSet } from './review-set'
import { collectImportedSymbols, type SliceRange, sliceSource } from './review-slice'
...
/**
 * A file plus the agent-declared flow layer that overrides the regex match. The
 * layer is a grouping INPUT, not a wire field: it names the group heading the file
 * lands under, so it is stripped once the groups are built.
 */
type LayeredActiveReviewFile = ActiveReviewFile & { layer?: string }

interface ActiveReviewGroup {
  layer: string
  files: ActiveReviewFile[]
}

/** Outline entry for one walkthrough section (the full section lives in the reading). */
interface ReviewSectionOutline {
  title: string
  anchorCount: number
}
...
export interface ActiveReview {
  ...
  sections: ReviewSectionOutline[]
  groups: ActiveReviewGroup[]
}
 ...
 */
export function buildActiveReview(params: {
  name: string
  ...
  const files: LayeredActiveReviewFile[] = []
  ...
  // regex layers still group Changes/History via `groupByLayer` in flow.ts. The layer
  // has done its job once the heading exists, so it does not travel on the wire.
  const groups = groupByLayerOrdered(files, params.layers).map((group) => ({
    layer: group.layer,
    files: group.files.map(({ layer: _layer, ...file }) => file),
  }))
...
export interface ReviewReading {
  ...
  groups: ReadingGroup[]
  /** Evidence meta when present; the pack itself is fetched by `reviewEvidence`. */
  evidence: {
    title: string
    updatedAt: string
...
// A ranged anchor renders at most this many lines — mirrors review-slice's
// MAX_TOTAL_LINES so one anchor can't dwarf the rest of the document.
const MAX_ANCHOR_LINES = 400

/**
 * Assemble the Review document from an already-built active review. Changed files
 ...
 */
export function buildReviewReading(params: {
  view: ActiveReview
  sections: readonly ReviewSection[]
  sources: ReadonlyMap<string, string>
  diffs: ReadonlyMap<string, DiffHunk[]>
  evidence: ReviewReading['evidence']
}): ReviewReading {
...
/**
 * The stacked-diff reading surface: a `ReviewReading` narrowed to what a pure
 ...
 */
export interface DiffReading extends ReviewReading {
  sections: []
  evidence: null
