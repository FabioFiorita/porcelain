...
// One shared build per snapshot — both review reading procedures reuse it instead of each
...
// working-tree change that affects the active review.
const reviewBuildCache = new Map<
  string,
  { key: string; view: ActiveReview; sources: Map<string, string> }
>()
...
// files (the review build parses imports off the contents).
...
// Cheap phase shared by both review reading procedures: the working-tree snapshot, agent
...
export type ReviewGather = Awaited<ReturnType<typeof gatherReview>> & { reviewSet: ReviewSet }
...
// then build the active review. Returns the view AND the sources (the reading
// surface needs them to slice context/shipped files).
async function buildActiveReviewFromGather(
  input: string,
  g: ReviewGather,
): Promise<{ view: ActiveReview; sources: Map<string, string> }> {
...
// or runs buildActiveReviewFromGather once and stores the result. Both feature
...
export async function getReviewBuild(
  input: string,
  g: ReviewGather,
): Promise<{ key: string; view: ActiveReview; sources: Map<string, string> }> {
  const cached = reviewBuildCache.get(input)
...
export function cachedReviewReading(repoPath: string, key: string): ReviewReading | null {
  const cached = reviewReadingCache.get(repoPath)
  return cached && cached.key === key ? cached.reading : null
}

export function storeReviewReading(repoPath: string, key: string, reading: ReviewReading): void {
  reviewReadingCache.set(repoPath, { key, reading })
}
