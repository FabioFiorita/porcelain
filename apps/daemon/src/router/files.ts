...
export const filesRouter = t.router({
  readFile: publicProcedure.input(z.string()).query(async ({ input }): Promise<FileView> => {
  ...
  createFile: publicProcedure.input(z.object({ path: z.string() })).mutation(async ({ input }) => {
