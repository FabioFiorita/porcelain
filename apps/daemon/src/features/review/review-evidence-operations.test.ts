...
describe('review evidence operations', () => {
  it('calls one store method per operation with the repo path the wire supplied', async () => {
    ...
    expect(await createReadEvidenceSummary({ store })({ projectPath: REPO })).toEqual(PACK)
    expect(await createReadEvidenceResults({ store })({ projectPath: REPO })).toEqual([DOC])
    expect(await createListEvidenceAssets({ store })({ projectPath: REPO })).toEqual(PACK.assets)
    expect(
  ...
  it('reads an absent pack as null without touching documents or assets', async () => {
    ...
    expect(await createReadEvidenceSummary({ store })({ projectPath: REPO })).toBeNull()
    expect(await createListEvidenceAssets({ store })({ projectPath: REPO })).toEqual([])
  ...
  it('propagates a rejecting store rather than collapsing it into null', async () => {
    ...
    await expect(createReadEvidenceSummary({ store })({ projectPath: REPO })).rejects.toThrow(
      'disk exploded',
    )
    await expect(createListEvidenceAssets({ store })({ projectPath: REPO })).rejects.toThrow(
      'disk exploded',
    )
  })
})
