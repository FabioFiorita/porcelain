 ...
 *
 * The roster is repo-scoped (`stores/terminals.ts`): these tabs belong to the checkout
 * you are in, which is the point — a place for quick build/test/run shells, not a
 * daemon-wide terminal manager. Saved Actions run here too, against the selected
 * Worktree, by spawning their named session into this roster.
 */
              ...
              return (
                <ContextMenu key={session.id}>
                  <ContextMenuTrigger
                    render={
                      <div
                        data-testid={TestIds.terminalTab(session.id)}
                        className={cn(
