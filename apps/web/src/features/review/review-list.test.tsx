...
// hands back a ReviewReading shaped exactly like the real reviewReading query.
vi.mock('./review-queries', () => ({
  useReviewReading: vi.fn(),
}))
// ReviewList mounts CommentComposer (right-click → "Comment on file"), which uses the
...
// ReviewList also renders the Review inbox above the outline; stub its Git workspace hook
...
describe('ReviewList', () => {
  beforeEach(() => {
    ...
    usePreferencesStore.setState({ sidebarTab: 'review' })
  ...
  it('opens the Review and jumps to a section from its chapter title', () => {
    ...
    const { tabs } = pane
    expect(tabs[0]).toMatchObject({ id: tabId('review', '/repo'), kind: 'review' })
