...
describe('hydrateViewerTabs', () => {
  ...
  it('drops a stale Terminals tab from a layout written before the panel, keeping the rest', () => {
    const stale = {
    ...
    const restored = hydrateViewerTabs({
      panes: [
        { tabs: [stale], activeTabId: 'terminals:terminals' },
        {
          tabs: [stale, { id: 'file:/repo/a.ts', kind: 'file', title: 'a.ts', path: '/repo/a.ts' }],
          activeTabId: 'terminals:terminals',
        },
      ],
      activePaneIndex: 1,
    })
    // The removed kind costs only its own tab; the pane it emptied collapses and the
    // surviving file is promoted, per the normal split-collapse rules.
    expect(restored.panes).toHaveLength(1)
    expect(restored.panes[0]?.tabs.map((t) => t.id)).toEqual(['file:/repo/a.ts'])
  })
