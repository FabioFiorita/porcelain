...
function requireLayoutField(
  ...
  if (!field) throw new Error(`libghostty-vt layout field is unavailable: ${name}`)
  return field
}
...
export type GhosttyMouseTrackingMode = 'none' | 'x10' | 'vt200' | 'drag' | 'any'
...
export class GhosttyTerminalCore {
  ...
  scroll(deltaRows: number): void {
    ...
    const value = requireLayoutField(layout, 'value')
  ...
  encodeMouse(input: GhosttyMouseInput): string {
    ...
    positionView.setFloat32(requireLayoutField(positionLayout, 'x').offset, input.x, true)
    positionView.setFloat32(requireLayoutField(positionLayout, 'y').offset, input.y, true)
  ...
  setSelection(anchor: GhosttyPointInput, end: GhosttyPointInput): void {
    ...
    try {
      ...
      const startField = requireLayoutField(selectionLayout, 'start')
      const endField = requireLayoutField(selectionLayout, 'end')
      this.runtime
  ...
  private gridRef(col: number, row: number, tag: 1 | 2 = 1): number {
    ...
    const pointValue = requireLayoutField(pointLayout, 'value')
  ...
  private selectAt(
    ...
    try {
      ...
      const refField = requireLayoutField(optionsLayout, 'ref')
      this.runtime
      ...
      if (result === GHOSTTY_SUCCESS) {
        const start = selection + requireLayoutField(selectionLayout, 'start').offset
        const end = selection + requireLayoutField(selectionLayout, 'end').offset
