...
import { isMonospaceFamily } from './monospace'
import {
...
export const DEFAULT_TERMINAL_FONT_FAMILY = `"SF Mono", "SFMono-Regular", Menlo, Consolas, "Liberation Mono", ${TERMINAL_GLYPH_FALLBACKS}`
...
export function terminalLinkAtPositionWithRange(
  ...
  const continuesBelowViewport = lastRow?.wrapsToNext ?? false
...
export interface GhosttyTerminalSurfaceOptions {
  ...
  /** Own the native paste event when the host needs rich clipboard items. */
...
export class GhosttyTerminalSurface {
  ...
  private constructor(
    ...
    this.resizeObserver = new ResizeObserver(() => this.resizeToMount())
    this.installEvents()
  ...
  static async create(
    ...
    canvas.className = 'porcelain-ghostty-canvas'
    canvas.style.cssText = 'display:block;width:100%;height:100%;'
    canvas.setAttribute('aria-hidden', 'true')

    const input = document.createElement('textarea')
    input.className = 'porcelain-ghostty-input'
    ...
    scrollbar.className = 'porcelain-ghostty-scrollbar'
    ...
    scrollbarThumb.className = 'porcelain-ghostty-scrollbar-thumb'
  ...
  /** Encode text according to the terminal's current bracketed-paste mode. */
  paste(text: string): void {
    if (this.disposed || text.length === 0) return
    const data = this.core.encodePaste(text)
  ...
  /** Test-only viewport serialization; never changes the user's selection. */
  textContentForTesting(): string {
    if (this.disposed || this.viewportCleared) return ''
  ...
  /** Read the live DECCKM mode for virtual key bars and touch scrolling. */
  applicationCursorKeys(): boolean {
    return !this.disposed && this.core.isApplicationCursorKeys()
  }

  /** Touch-scroll adapter entrypoint; alternate screens receive terminal keys. */
  scrollTouch(lines: number): void {
    if (this.disposed || lines === 0) return
    applyTerminalTouchScroll(
      {
        bufferType: this.core.isAlternateScreen() ? 'alternate' : 'normal',
        mouseTrackingMode: this.core.mouseTrackingMode(),
        cols: this.cols,
        rows: this.rows,
        scrollLines: (delta) => this.scrollViewport(delta),
        input: (data) => this.options.onData(data),
      },
      lines,
    )
  }
  ...
  /**
   * Clear only the local presentation until the next PTY output. This mirrors
   * xterm's clear-viewport action without injecting RIS/ED into a remote TUI.
   */
  clearViewport(): void {
    if (this.disposed) return
    this.viewportCleared = true
    this.context.save()
    this.context.resetTransform()
    this.context.fillStyle = `rgb(${this.theme.background.r}, ${this.theme.background.g}, ${this.theme.background.b})`
  ...
  private readonly onPaste = (event: ClipboardEvent) => {
    ...
    event.preventDefault()
    if (this.options.onPaste !== undefined) {
      this.pasteShortcutToken += 1
      this.options.onPaste(event)
      return
    }
    ...
    applyTerminalTouchScroll(
      {
        ...
        cols: this.cols,
        rows: this.rows,
        scrollLines: (lines) => this.scrollViewport(lines),
        input: (data) => this.options.onData(data),
      },
      delta.rows,
    )
