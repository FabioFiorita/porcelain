...
describe('vendored libghostty-vt WebAssembly', () => {
  ...
  it('uses Ghostty for mouse encoding, word selection, and OSC 8 hit testing', async () => {
    ...
    new Uint8Array(memory.buffer, mousePositionPointer, 8).set(new Uint8Array(mousePosition.buffer))
  ...
  it('encodes modified printable keys in Kitty keyboard mode', async () => {
    ...
    call('ghostty_key_event_set_unshifted_codepoint', keyEvent, 'c'.codePointAt(0) ?? 0)
    ...
    call('ghostty_key_event_set_unshifted_codepoint', keyEvent, 'j'.codePointAt(0) ?? 0)
