...
# Adapted from T3 Code's Ghostty web build script (MIT, Copyright 2026 T3
# Tools Inc.). Full attribution is in src/terminal/ghostty/NOTICE.md.
...
GHOSTTY_SOURCE_DIR="${GHOSTTY_SOURCE_DIR:-${HOME}/.cache/porcelain/ghostty-${GHOSTTY_REVISION:0:8}}"
...
ensure_zig() {
  ...
  cache_dir="${HOME}/.cache/porcelain/zig-${GHOSTTY_ZIG_VERSION}"
  ...
  curl -fsSL -o "${signature}" \
    "https://ziglang.org/download/${GHOSTTY_ZIG_VERSION}/zig-${host_arch}-${host_os}-${GHOSTTY_ZIG_VERSION}.tar.xz.minisig"
...
(
  ...
  # the single source of truth for the pin.
  "${GHOSTTY_ZIG}" build \
    -Demit-lib-vt \
