...
it('rejects invalid configuration before creating state', async () => {
  ...
  try {
    ...
    await expect(
      startLocalServer({ dataDirectory, token, port: 0 }, AbortSignal.abort()),
    ).rejects.toThrow();
    await expect(
...
it('owns a directory across aliases, serves loopback HTTP, and releases it on close', async () => {
  ...
  try {
    ...
    const restarted = await startLocalServer({ dataDirectory, token, port: 0 });
    await restarted.close();
  } finally {
    await server.close();
    await rm(root, { recursive: true, force: true });
...
it('releases ownership after bind or database failure so startup can be retried', async () => {
  ...
  try {
    ...
    await retry.close();
    await writeFile(join(dataDirectory, 'inventory.sqlite'), 'invalid sqlite');
    await expect(
