...
describe('Playground workflow', () => {
  it('starts an explorable server, accepts a documented comment and removes its data on shutdown', async () => {
    const child = spawn(
      process.execPath,
    ...
    expect(
      commentThreadsSchema.parse(await (await fetch(url, { headers })).json()),
    ).toEqual([thread]);
    child.kill('SIGTERM');
    await vi.waitFor(() => expect(child.exitCode).toBe(0), { timeout: 5000 });
    expect(await exited).toBe(0);
    await expect(access(info.tokenFile)).rejects.toThrow();
  }, 20000);
});
