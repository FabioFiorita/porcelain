...
export const openServer: OpenServer = async (input) => {
  ...
  const session = openStorageSession(settings.dataDirectory, { worktreeId });
  ...
  const { worktreeDirectory } = shared;
  const context: ComposeContext = {
    lanes,
    laneKeys: new GitLaneKeys(worktreeDirectory, stores.inventory),
    events,
    settings,
    clock,
