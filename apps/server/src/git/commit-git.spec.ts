...
async function reader(
  ...
  if (!worktree?.metadataIdentity) throw new Error('Missing fixture identity');
  const checkout = {
    path,
...
it('detects renames at 50 percent, retains literal unusual paths, and distinguishes binary and modes without modifying the checkout', async () => {
  ...
  expect(result.changes).toEqual(
    expect.arrayContaining([
      ...
      expect.objectContaining({ newPath: 'binary', patch: { kind: 'binary' } }),
  ...
  expect(await readFile(join(f.path, 'untracked'), 'utf8')).toBe('leave me');
});
...
it('truncates multibyte subjects at a valid UTF-8 boundary and rejects oversized change results', async () => {
  ...
  await expect(f.adapter.inspectCommitChanges({ oid })).rejects.toBeInstanceOf(
    ReadLimitExceededError,
  );
  for (let i = 0; i < 501; i++) await writeFile(join(f.path, `file-${i}`), 'x');
  git(f.path, 'add', '.');
  git(f.path, '-c', 'commit.gpgsign=false', 'commit', '-m', 'many');
  await expect(
    f.adapter.inspectCommitChanges({ oid: git(f.path, 'rev-parse', 'HEAD') }),
...
it('ignores replacement objects and configured external diff and textconv programs', async () => {
  ...
  expect(inspection.comparison).toEqual({ kind: 'empty-tree' });
  expect(inspection.changes.map((entry) => entry.newPath)).toEqual(['file']);
  await expect(
...
it('supports octopus parent selection and reports gitlinks, mode changes and copies without recursion', async () => {
  ...
  git(f.path, 'update-index', '--add', '--cacheinfo', `160000,${root},module`);
  git(f.path, 'update-index', '--chmod=+x', 'root');
  await writeFile(join(f.path, 'copy'), 'root');
  git(f.path, 'add', 'copy');
  git(f.path, '-c', 'commit.gpgsign=false', 'commit', '-m', 'gitlink and mode');
...
it('does not lazily fetch promised blobs during commit inspection', async () => {
  ...
  const { adapter } = await reader(path);
  const missing = git(path, 'rev-list', '--objects', '--missing=print', 'HEAD');
  ...
  git(path, 'config', 'remote.origin.uploadpack', `touch '${marker}'; false`);
  await expect(adapter.inspectCommitChanges({ oid })).rejects.toBeInstanceOf(
    HistorySnapshotUnavailableError,
  );
  expect(git(path, 'rev-list', '--objects', '--missing=print', 'HEAD')).toBe(
    missing,
  );
