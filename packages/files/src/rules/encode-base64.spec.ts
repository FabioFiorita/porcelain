...
describe('encodeBase64', () => {
  ...
  it('encodes bytes above 127 as raw bytes rather than as text', () => {
    expect(
      encodeBase64(new Uint8Array([0x00, 0xff, 0xfe, 0x80]), CHUNK_BYTES),
    ).toBe('AP/+gA==');
  });

  it('keeps every byte in order across content larger than a chunk', () => {
    const bytes = Uint8Array.from(
      { length: CHUNK_BYTES * 2 + 7 },
      (_, index) => (index * 31) % 256,
    );
    expect(bytesOf(atob(encodeBase64(bytes, CHUNK_BYTES)))).toEqual(bytes);
    expect(encodeBase64(bytes, 3)).toBe(encodeBase64(bytes, CHUNK_BYTES));
  });
});
