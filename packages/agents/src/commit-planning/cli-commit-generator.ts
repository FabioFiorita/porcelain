...
import { AgentGenerationError } from './agent-generation-error.ts';
...
export class CliCommitGenerator {
  ...
  async generate(
    ...
    if (
      ...
      throw new AgentGenerationError('Unsupported commit model.');
    const command = await executable(provider === 'codex' ? 'codex' : 'claude');
    if (!command)
      throw new AgentGenerationError(
        'The selected coding CLI is not installed.',
      );
    ...
    try {
      ...
      signal.throwIfAborted();
      if (cause instanceof AgentGenerationError) throw cause;
      throw new AgentGenerationError('Commit generation failed.', { cause });
    } finally {
