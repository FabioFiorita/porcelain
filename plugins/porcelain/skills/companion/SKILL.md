...
handoff. A normal Canvas accepts Markdown/HTML files or a daemon-host directory. Prefer the
semantic `decision` template for an RFC or choice: concise `templateData` supplies context,
options, criteria, assessments, recommendation, and an optional recorded decision while Porcelain
owns presentation. The `review` template accepts structured `templateData` with a title, Why/How blocks, review layers, declared files, and Assets;
