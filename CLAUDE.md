...
**Agent scratch + test debris:** `scripts/agent-scratch/` is **gitignored** for throwaway automation (screenshot harnesses, seed helpers, one-off verify scripts, local PNG dumps) — put it there, not under tracked `scripts/` or the repo root. Prefer reusing a named harness over re-inventing the same daemon+Chromium bootstrap each time. **Clean up after yourself before you stop:** delete session-local test/browser debris you created under the repo (notably **`.playwright-mcp/`** — Playwright MCP writes `page-*.yml` / console logs on every snapshot; also stray files under `test-results/`, `playwright-report/`, `e2e/.artifacts/`, or `scripts/agent-scratch/` when the session no longer needs them). Those paths are gitignored so they never ship, but leaving piles of yml/log dumps is still noise for the human. Do not leave untracked junk in the working tree at session end.
