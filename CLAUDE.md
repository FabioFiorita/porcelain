...
- **Tab kinds** — `file view` / `source view` (`source-view.tsx`) / `markdown reader` (`markdown-view.tsx`) / `diff view` (`diff-view.tsx`) / `commit view` (`commit-view.tsx`) / `search view` (`search-view.tsx`) / `feature view` (`feature-view.tsx`) / `explore view` (`explore-view.tsx`) / `references view` (`references-view.tsx`, opt-in LSP) / `board view` (`board-view.tsx`) / `terminal view` (`terminal-view.tsx`). What each renders → read the file; the concepts → `product`.
