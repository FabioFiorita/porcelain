...
- **Top bar** — the full-width window **titlebar** (`title-bar.tsx`): traffic lights on the left, a centered **search button** that raises the file finder (⌘K/⌘P) — nothing else. On Linux/Windows the titlebar instead carries a **Window controls** cluster (min/max/close, `window-controls.tsx`) in place of the macOS OS traffic lights. The viewer's own header (`TopBar` in `app-shell.tsx`, below the titlebar) carries the sidebar toggle + tabs + Update pill + Quick-Access bolt.
