 ...
 * Empty → agent starts Intent; in progress → Execution/Evidence grow;
 * ready → human can close (ship via Changes).
 */
...
export function reviewStartPrompt(options?: { name?: string }): string {
  ...
  const nameHint = name && name.length > 0 ? name : '<short name: bug | feature | chore>'
  return [
