...
// Mock the domain hooks (never tRPC) and the toast system; the component is pure
...
beforeEach(() => {
  ...
  vi.mocked(useCodexInfo).mockReturnValue(undefined)
...
describe('PluginUpdateToast', () => {
  it('raises a toast when the bundled plugin is newer than the installed one', () => {
    ...
    expect(opts?.description).toContain('/reload-plugins')
  })
  ...
  it('raises a Codex-worded toast when only the Codex plugin is behind', () => {
    vi.mocked(usePluginInfo).mockReturnValue(undefined)
    vi.mocked(useCodexInfo).mockReturnValue(pluginInfo('2.4.0'))
    usePreferencesStore.setState({
      pluginInstalled: false,
      pluginVersion: null,
      codexPluginInstalled: true,
      codexPluginVersion: '2.3.0',
    })
    render(<PluginUpdateToast />)
    expect(toast.info).toHaveBeenCalledTimes(1)
    const [, opts] = vi.mocked(toast.info).mock.calls[0]
    expect(opts?.description).toContain('v2.4.0')
    expect(opts?.description).toContain('start a new Codex thread')
  })

  it('records the Codex dismissal from the action when only Codex is pending', () => {
    vi.mocked(usePluginInfo).mockReturnValue(undefined)
    vi.mocked(useCodexInfo).mockReturnValue(pluginInfo('2.4.0'))
    usePreferencesStore.setState({
      pluginInstalled: false,
    ...
    const action = lastToast().action as ToastAction
    action.onClick()
    expect(usePreferencesStore.getState().codexPluginUpdateDismissedVersion).toBe('2.4.0')
    expect(usePreferencesStore.getState().pluginUpdateDismissedVersion).toBeNull()
  })
})
