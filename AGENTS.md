...
3. **Verification gate before any commit:** `pnpm verify` (= `pnpm lint && pnpm test && pnpm build`; typecheck runs inside `build`). **Hook-enforced for every client** — the tracked `.husky/pre-commit` is authoritative (husky wires `core.hooksPath` from `prepare`); Claude Code and Grok Build also run the shared `.agents/hooks/git-guard.sh` earlier through `.claude/settings.json`. Codex reaches the same tracked hook when it commits. Any path blocks the commit until the gate passes. `.husky/commit-msg` additionally gates the **message**: `type(scope): summary` ≤72 chars, blank line 2, **whole message ≤1024 chars** — EAS rejects a longer build/update message and the mobile delivery run dies. Conventions and the why live in `close-the-loop`.
...
| Commit gate | `.husky/pre-commit` | yes | yes | yes |
| Commit-message gate | `.husky/commit-msg` | yes | yes | yes |
...
- **`.husky/pre-commit`** — authoritative client-independent rule-3 gate, wired by `pnpm install`; Grok always reaches it even after the early guard because session presence alone cannot prove the trusted project hook succeeded. Claude Code retains its host-guaranteed duplicate skip; `PORCELAIN_SKIP_VERIFY=1` is the explicit manual escape.
- **`.husky/commit-msg`** — runs `scripts/lint-commit-message.mjs`. No pnpm, no network, no skip: it runs even on the commits where `pre-commit` skipped verification, because the 1024-character ceiling is EAS's, not ours.
- **Husky owns the wiring only.** It sets `core.hooksPath=.husky/_` from `prepare` and generates the shims there (gitignored, regenerated by every install); the hook *bodies* are tracked in `.husky/` and are what actually runs. Two consequences worth knowing: husky's shim sources `~/.config/husky/init.sh` if it exists — a machine-local file in the commit path, never a place for project rules — and **`HUSKY=0` skips every hook**. Treat that as an alias for `--no-verify`: same blast radius, same expectation that you ran the gate yourself.
