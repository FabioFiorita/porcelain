...
4. **Docs say what the code can't.** A skill carries decisions, the why, the deliberately-absent, and the traps — never a paraphrase of how a file works today. When a skill only restates code, *cut it*. Prefer a lint over prose whenever a lint can enforce the rule.
...
Shell regions, the seven sidebar tabs (**Files · Changes · Review · History · Search · Board · Terminal**, ⌘1–7), viewer tab kinds, and cross-cutting terms live in the **`architecture` skill → Nomenclature** (term → entry file). When the human uses a bare noun ("the viewer", "the Changes tab"), resolve it there and act — don't re-ask.
