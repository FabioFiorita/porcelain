...
Porcelain's domain-first architecture is landed. The codebase has ten canonical product
...
tasks · actions · terminal · project-data · remote
```

`tasks` is the only one with no repo-local storage at all: the Tasks table belongs
...
The live registry is `scripts/architecture/domains.mjs`; all ten domains are complete, with
