...
Health, inventory, and file preference HTTP schemas live in `packages/contracts`;
[the HTTP decision](decisions/0004-inventory-http.md) defines the inventory boundary.
[File preferences](decisions/file-preferences.md) use a separate repository and table to retain human intent across inventory refresh.
