...
Dependencies are added with their first use. The unused Markdown and Store packages were removed
during the server/web cleanup; rendered Markdown and shared presentation state can be considered
when product behavior requires them.

React Doctor runs explicitly and in CI. We removed the blocking commit hook to keep local
iteration focused. Vendor exclusions remain in its configuration; the score is not a shipping
guarantee.


## Presentation state and content libraries

Component-local state stays in React, URL state stays in Router, and server data stays in Query.
A separate presentation store is deferred until a concrete shared-state requirement exists.
