...
availability. Unknown identity is a safe 400 INVALID_REQUEST. The existing single trusted
...
Paths must already be canonical, slash-separated relative paths. Empty components, dot and
traversal components, backslashes, colons, NUL, absolute paths, and case-insensitive `.git`
components are rejected. Clients must submit this spelling; the server never resolves paths
...
Preference operations share application serialization, deadlines, and shutdown behavior.
