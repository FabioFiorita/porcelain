...
- Worktrees/branch: `gitBranch` (rev-parse, 5s poll) + `gitWorktrees` (`worktree list --porcelain`, parser in diff.ts); `WorktreeSwitcher` in the sidebar footer (shadcn `dropdown-menu`) clears tabs and `openRepoPath`s the chosen worktree.
